   ...
   try {
      await db.Complaint.findAll({
         include: [
            ...
            {
               model: db.complaint_result,
               as: "complaint_result",
               attributes: ["id", "status", "message", "estimated_time"],
            },
            {
...
const destroyComplaint = async (req, res) => {
   try {
      ...
      await db.Complaint.destroy({ where: { id: id } }).then((res) => db.complaint_result.destroy({ where: { complaintId: id } }));
...
module.exports = { storeComplaint, getComplaint, destroyComplaint, getComplaintById, updateComplaint, getComplaintByUser };
